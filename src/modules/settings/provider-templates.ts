...
export const providerTemplates: ProviderTemplate[] = [
  ...
  localTemplate({
    ...
    models: ["local-model"],
    note: "LocalAI exposes an OpenAI-compatible local server; it is keyless by default and uses the configured local endpoint.",
  }),
  localTemplate({
    id: "llama-cpp",
    ...
    apiBaseUrl: "http://127.0.0.1:8080/v1",
    models: ["local-model"],
    note: "llama.cpp server exposes an OpenAI-compatible local route with no provider API key by default.",
  }),
  localTemplate({
    id: "vllm",
    label: "vLLM",
    ...
    models: ["local-model"],
    note: "vLLM local serving is configured as a keyless local runtime with an editable OpenAI-compatible endpoint.",
  }),
  localTemplate({
    ...
    models: ["local-model"],
    note: "Text Generation WebUI can expose an OpenAI-compatible local endpoint; no provider API key is required by default.",
  }),
  localTemplate({
    id: "dgx-spark",
