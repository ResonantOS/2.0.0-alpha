...
| `browser-first/host/dev-external-agent-runtimes-panel.mjs` | Opt-in development enumeration of `examples/addons/*.json` and the whitelisted manifest projection served on the `/dev/external-agent-runtimes` routes | Add-on example manifests under the launcher-provided repository root | Nothing; the panel is observational | Bridge authentication or capability enforcement (stays in `bridge-server.mjs`), route registration policy (stays in `run-bridge-minimal.mjs`), provider launches, grant minting, file writes, or production availability |
| `browser-first/dev/external-agent-runtimes-panel.html` | Static dev-panel presentation rendered entirely from server-injected inert data | The injected manifest projection | Nothing; the page issues no client-side bridge requests | Bridge secrets, capability policy, or host state |
