...
| `browser-first/host/harness-credentials.mjs` and `harness-transport.mjs` | Named operator-approved credential bindings, ephemeral secret custody, and authenticated DSH HTTP/WebSocket client transport | Host configuration, bounded private token files/environment, and fresh loopback DNS answers | Host-only tokens/cookies and requests to the approved origin/port | Add-on consent, slot ownership, adapter semantics, bridge routes, browser credentials, or UI state |
