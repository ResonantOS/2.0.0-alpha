...
| `browser-first/resonantos-side-panel-extension/src/lib/main-workspace-opencode*.js`, `opencode-bridge-source.js`, `opencode-session-model.js`, and `opencode-session-view.js` | Optional OpenCode governance/evidence presentation, one session-bound bridge poller, and truthful start/prompt/once-or-reject/stop controls | Redacted host readiness, redacted workspace label, attributable bounded events, and session identifier | Extension-local cursor, reducer, transcript, approval, and evidence-view state | Raw OpenCode URLs or credentials, direct OpenCode fetches, process/provider authority, persistent grants, fake revert, or Alpha runtime requirements |
...
| `browser-first/host/run-bridge-minimal.mjs` and `owned-shutdown-retry.mjs` | Bridge composition root, route-service wiring, token creation, startup, shutdown, and persistent retry while an owned child has not confirmed termination | Service constructors, environment configuration, and owned-service cleanup results | Listener lifecycle, generated bridge config, and bounded shutdown retry state | Domain route behavior that belongs in a service or control of a foreign process |
...
| `browser-first/host/opencode-client.mjs`, `opencode-session-host-service.mjs`, and `opencode-session-composition.mjs` | Optional governed OpenCode SDK access, authenticated child lifecycle, one owned session, and bounded event relay | Host-owned OpenCode consent, selected workspace/model/provider environment, and attributable SDK events | One bridge-started loopback child, one SDK session, permission replies, and in-memory event state | Caller-selected authority, shell or process tools in live preview, foreign process control, raw service credentials/URLs in the extension, persistent permission grants, or Alpha runtime requirements |
...
| `browser-first/host/addon-delegation-host-service.mjs`, `addon-delegation-service.mjs`, `hermes-runtime.mjs`, and `opencode-runtime.mjs` | Optional local add-on status/control, persisted consent and live-session preflight, fixed-root runtime resolution, scoped delegation, artifacts, and goals | Reviewed add-on manifests, bounded workspace state, provider availability, persisted add-on settings, and fixed runtime install roots | Validated add-on settings, capability-gated local runtime inputs, and delegation records | Core bridge authentication, ambient command lookup, raw provider secrets in responses, direct child ownership, wallet actions, or trusted-memory bypasses |
